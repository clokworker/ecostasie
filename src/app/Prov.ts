...
export class Provs{
    ...
    Nm:string='';Dir:string='';r:string='';C:string='';Estado:string='';ID:string;CP:string='';RFC:string='';ena:boolean=true;Conts:string; Edit:boolean=false;
    ...
    Tabler(){
        ...
        this.H.post('/api', JSON.stringify({"ProLst":1}),{headers:h}).map(R=>{
            ...
            R.json().forEach(I => {
                ...
                I.Contactos = I.Contactos == null ? '' : I.Contactos.replace('|','\n').slice(0,-1);
                this.Pro.push(I);
                console.log(I.Contactos);
            });
        }).subscribe();
    ...
    Registerer(){
        ...
        this.Conts.slice(0,-1);
        let H = new Headers();
        H.append('Content-Type', 'application/json');
        if(this.Edit){
            this.H.post('/api', JSON.stringify({"ProUpd":1, "Alias":this.Nm, "Razon":this.r, "Direccion":this.Dir, "Ciudad":this.C, "Estado":this.Estado, "RFC":this.RFC, "CP":this.CP, "Contactos":this.Conts, "ID":this.ID}),{headers:H}).map(R=>{
                console.log(R.json())
            ...
            this.H.post('/api', JSON.stringify({"ProIns":1, "Alias":this.Nm, "Razon":this.r, "Direccion":this.Dir, "Ciudad":this.C, "Estado":this.Estado, "RFC":this.RFC, "CP":this.CP, "Contactos":this.Cont}),{headers:H}).map(R=>{
                console.log(R.json())
                if(!R.json().DatabseError){
                    if(R.json().affectedRows='1'){
                        this.Tabler();
        ...
        this.Dir="";this.Nm="";this.C="";this.Estado="";this.RFC="";this.CP="";
        this.B=[];
        this.Cont=[];
        this.Mails=[];
        this.Cels=[];
    ...
    EditPro(P){
        ...
        this.Estado=P.Estado;this.CP=P.CP;this.RFC=P.RFC; let Cont= P.Contactos==null ? [''] : P.Contactos.split('|'); this.ID=P.id;this.Edit=true;
        this.B=[];
        this.Cont=[];
        this.Mails=[];
        this.Cels=[];
    ...
    ElimPro(P){
        ...
        this.c.confirm({
            ...
            accept: ()=>{
                ...
                let H = new Headers();
                H.append('Content-Type', 'application/json');
                this.H.post('/api', JSON.stringify({"ProDel":1, "ID":P.id}), {headers:H}).map(R=>{
                    console.log(R.json())
                    if(!R.json().DatabseError){
                        if(R.json().affectedRows='1'){
                            this.Tabler();
